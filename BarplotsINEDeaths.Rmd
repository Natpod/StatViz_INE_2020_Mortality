...
monthccaa <- ggplot(data = df, mapping = aes(x=CCAA,y=Deaths))+
  ...
  ggtitle("Crude Mortality per CCAA in 2020")+
  theme(plot.title = element_text(size=16))+
...
## Cálculo Death Rate por mes y provincia
...
monthsex <- ggplot(data = df, mapping = aes(x=Province,y=DeathRate))+
  geom_boxplot()+
  ylab("Death Rate (per 100.000 inhabitants)")+
...
monthccaa <- ggplot(data = df, mapping = aes(x=CCAA,y=DeathRate))+
  geom_boxplot()+
  ggtitle("Death Rate per CCAA in 2020")+
  ylab("Death Rate (per 100.000 inhabitants)")+
  xlab("Autonomous Community")+
  theme(plot.title = element_text(size=16))+
  theme(axis.text.x = element_text(angle=45, hjust=1, size = 9),
